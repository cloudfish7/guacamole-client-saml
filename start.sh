...
associate_header() {
    ...
    # Add required .jar files to GUACAMOLE_EXT ln -s /opt/guacamole/header/guacamole-auth-*.jar "$GUACAMOLE_EXT"
}
...
associate_saml() {
    ...
    # Verify required parameters are present
    if [ -z "$SAML_CALLBACK_URL" ]; then
        cat <<END
FATAL: Missing required environment variables
-------------------------------------------------------------------------------
If using an SAML directory, you must provide each of the following environment
variables:

    SAML_CALLBACK_URL   The URL that the IdP will use once authentication has 
                        succeeded to return to the Guacamole web application 
                        and provide the authentication details to the SAML 
                        extension. The SAML extension currently only supports
                        callback as a POST operation to this callback URL. 
...
END
    ...
    set_optional_property "saml-group-attribute"             "$SAML_GROUPATTRIBUTE"
